...
class AOTBlock(nn.Module) :
	def __init__(self, planes, alpha, beta) :
		super(AOTBlock, self).__init__()
		assert planes % 4 == 0
		self.alpha = alpha
		self.beta = beta
		quarterplanes = planes // 4
		self.conv_r1 = GatedWSConvPadded(planes, quarterplanes, 3, 1, 1)
		self.conv_r2 = GatedWSConvPadded(planes, quarterplanes, 3, 1, 2)
		self.conv_r4 = GatedWSConvPadded(planes, quarterplanes, 3, 1, 4)
		self.conv_r8 = GatedWSConvPadded(planes, quarterplanes, 3, 1, 8)
		self.proj = GatedWSConvPadded(planes, planes, 3, 1, 1)
		self.gate = GatedWSConvPadded(planes, planes, 3)
		self.relu = LambdaLayer(gelu_nf)

	def forward(self, x) :
		x = x / self.beta
		r1 = self.relu(self.conv_r1(x))
		r2 = self.relu(self.conv_r2(x))
		r4 = self.relu(self.conv_r4(x))
		r8 = self.relu(self.conv_r8(x))
		proj = self.proj(torch.cat([r1, r2, r4, r8], dim = 1))
		gate = my_layer_norm(self.gate(x)).sigmoid()
		return x * (1 - gate) + proj * gate
